...
%
\begin{abstract}
Classification of electroencephalogram-based motor imagery (MI-EEG) tasks is crucial in brain-computer interface (BCI). EEG signals require a large number of channels in the acquisition process, which hinders its application in practice. How to select the optimal channel subset without a serious impact on the classification performance is an urgent problem to be solved in the field of BCIs. This article proposes an end-to-end deep learning framework based on graph convolutional neural networks (GCN) with an adaptive gating mechanism to fully exploit the correlation of signals in the temporal and spatial domains. Edge-selection (ES) and aggregation-selection (AS) channel selection methods, combined with gate-selection (GS) derived from the adaptive gates, are employed to select a specified number of optimal channels automatically. Experiments on the PhysioNet EEG Motor Movement/Imagery dataset with 10 subjects demonstrate that the adaptive gating model outperforms the previous baseline architecture. Using only 10-20 channels, we obtain classification performance comparable to using all 64 channels, with average accuracy drops of only 2-4\%. The adaptive gating method achieves the best full-channel performance (84.60\%), and the selected channels align with sensorimotor cortex locations, validating the neurophysiological relevance of our approach.
...
We propose an end-to-end EEG-ARNN framework that brings together temporal feature extraction and graph convolutional reasoning to learn channel connectivity automatically, demonstrating that adaptive gating delivers the strongest results when combined with the baseline architecture. The adaptive gating module learns per-trial channel importance, supporting gate-selection (GS) for channel reduction, while ES and AS provide complementary adjacency-based views. Together, these components allow us to retrain on 1/6 to 1/3 of the original channels with minimal accuracy loss. Our experiments on 10 PhysioNet subjects confirm the effectiveness of this pipeline, showing that selected channels align with known neurophysiological regions while the classification performance remains competitive.
...
Following the determination of the top \textbf{k} channels via \textbf{ES}, \textbf{AS}, or \textbf{GATE} techniques, we reset the model parameters and perform training from the beginning, using inputs drawn solely from these designated electrodes. This end-to-end retraining step enables the architecture to generate novel feature embeddings and adjust inter-node dependencies within the graph framework to align with the downsized electrode layout, typically producing enhanced signal processing and linkage configurations that are better matched to the smaller-scale data. During this phase, the core design of the network, the chosen loss criterion, the update mechanism, and all tuning parameters mirror those employed in the primary full-channel experiments—the sole alteration being the shift in input size from the complete electrode array ($\textbf{C = 64}$) to the specified $\textbf{k}$. Accordingly, the trained system handles just the condensed dataset segment, promoting resilient recalibration that may regain or sometimes improve upon the reference accuracy by filtering out irrelevant disturbances and channeling resources toward the primary neural indicators linked to motor imagery activity.
...
To ensure robust results, we performed comprehensive data quality screening, excluding subjects with fewer than eight task runs, a high proportion of clipped or artifactual epochs, or known labeling/recording problems.
...
Raw EDF recordings were preprocessed with MNE-Python \cite{mne}: we cleaned channel names and restricted to EEG electrodes, applied the standard 10-20 montage, removed 50 Hz powerline noise via notch filtering, and filtered between 0.5--40 Hz with an FIR filter. We also applied common-average reference (CAR), downsampled to 128 Hz, extracted -1.0s to 5.0s epochs around task cues, and performed baseline correction using the -0.5s to 0s pre-stimulus window.
...
All models were trained using 3-fold stratified cross-validation to keep class balance across folds. We used a hidden dimension of 40, trained for up to 20 epochs per fold with batch size 64, and optimized with Adam (learning rate $10^{-3}$, weight decay $10^{-4}$) coupled with ReduceLROnPlateau (factor 0.5, patience 3), early stopping after 8 epochs, and dropout of 0.5 in the fully connected layers. The gating module shares the same $L_1$ regularization ($\lambda_{\text{gate}} = 10^{-3}$) and is initialized with a bias that places the gates near 0.9 so that adaptation can drive sparsity gradually.

\subsection{Channel Selection Evaluation}

For each subject and model variant, we trained on all 64 channels with 3-fold cross-validation, extracted the learned adjacency and gate values, and used ES, AS, or GS to select $k \in \{10,15,20,25,30\}$ channels. The full model was then retrained on the reduced set and evaluated by comparing accuracy drop to the full-channel baseline.
...
\begin{table}[t]
\centering
\caption{Full-channel classification accuracy (mean $\pm$ std across 3 folds) for baseline/adaptive gating and reference literature methods (not calculated per subject).}
\label{tab:full_channel}
\begin{tabular}{lcccccccc}
\toprule
Subject & Baseline & Adaptive Gating & Gated EEG-ARNN & FBCSP & CNN-SAE & EEGNet & ACS-SE-CNN & G-CARM \\
\midrule
S001 & 0.8849 $\pm$ 0.022 & 0.8214 $\pm$ 0.039 & 0.8214 $\pm$ 0.039 & n/c & n/c & n/c & n/c & n/c \\
S002 & 0.7540 $\pm$ 0.028 & 0.7579 $\pm$ 0.025 & 0.7579 $\pm$ 0.025 & n/c & n/c & n/c & n/c & n/c \\
S005 & 0.8175 $\pm$ 0.054 & 0.8214 $\pm$ 0.017 & 0.8214 $\pm$ 0.017 & n/c & n/c & n/c & n/c & n/c \\
S006 & 0.7698 $\pm$ 0.020 & 0.7976 $\pm$ 0.035 & 0.7976 $\pm$ 0.035 & n/c & n/c & n/c & n/c & n/c \\
S007 & 0.9365 $\pm$ 0.025 & 0.9206 $\pm$ 0.034 & 0.9206 $\pm$ 0.034 & n/c & n/c & n/c & n/c & n/c \\
S008 & 0.8889 $\pm$ 0.020 & 0.9048 $\pm$ 0.039 & 0.9048 $\pm$ 0.039 & n/c & n/c & n/c & n/c & n/c \\
S011 & 0.8016 $\pm$ 0.020 & 0.8016 $\pm$ 0.020 & 0.8016 $\pm$ 0.020 & n/c & n/c & n/c & n/c & n/c \\
S014 & 0.8571 $\pm$ 0.054 & 0.8889 $\pm$ 0.020 & 0.8889 $\pm$ 0.020 & n/c & n/c & n/c & n/c & n/c \\
S015 & 0.8611 $\pm$ 0.049 & 0.8889 $\pm$ 0.031 & 0.8889 $\pm$ 0.031 & n/c & n/c & n/c & n/c & n/c \\
S016 & 0.8413 $\pm$ 0.059 & 0.8571 $\pm$ 0.054 & 0.8571 $\pm$ 0.054 & n/c & n/c & n/c & n/c & n/c \\
\midrule
Mean & 0.8413 $\pm$ 0.057 & \textbf{0.8460 $\pm$ 0.054} & \textbf{0.8460 $\pm$ 0.054} & n/c & n/c & n/c & n/c & n/c \\
\bottomrule
\end{tabular}
\end{table}

Key observations include the robustness of both baseline and adaptive gating, with the latter attaining the highest mean accuracy (84.60\%) and indicating that input-dependent channel weighting provides a measurable benefit. Subject S007 sustains excellent performance (above 92\%), whereas S002 continues to pose the greatest challenge (around 75-78\%), underscoring the inter-subject variability even when using the same architecture.

\subsection{Channel Selection Performance}

Adaptive gating provides a gate-selection (GS) signal that allows us to pick top-$k$ electrodes with minimal accuracy loss. From the aggregated results (Table 1 in \texttt{results/channel\_selection\_analysis/aggregated\_results.csv}), keeping around 20 channels selected via gate averages results in an average drop of $\approx$0.6\%, while ES/AS on the baseline model drop 3--4\%. This highlights that gate-based selection from adaptive gating yields the most compact yet accurate channel subsets.

Retraining on the reduced subsets stabilizes performance and ensures the graph decoder adapts to the smaller input space.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{results/channel_selection_analysis/accuracy_drop_vs_k.png}
  \caption{Accuracy drop versus channel count for ES, AS, and gate-based selection across the models.}
  \label{fig:drop_vs_k}
\end{figure}

\begin{table}[t]
\centering
\caption{Comparative accuracy and reported F1 scores (where available). Literature values for external methods are taken from their original publications; n/r denotes values not reported per subject.}
\label{tab:literature}
\begin{tabular}{lcc}
\toprule
Method & Accuracy & F1 Score \\
\midrule
FBCSP \cite{chansel1} & 0.78 (aggregate) & n/r \\
CNN-SAE \cite{cnn1} & 0.80 (aggregate) & n/r \\
EEGNet \cite{eegnet} & 0.79 (aggregate) & n/r \\
ACS-SE-CNN \cite{csp2} & 0.81 (aggregate) & n/r \\
G-CARM \cite{gcn2} & 0.83 (aggregate) & n/r \\
EEG-ARNN (Baseline, this work) & 0.84 (mean over 10 subjects) & n/r \\
Gated EEG-ARNN (Adaptive gating, this work) & 0.846 (mean) & TBD \\
\bottomrule
\end{tabular}
\end{table}

% tanishq results analysis starts here
\subsection{Results of GS, AS and ES}

To further investigate the performance of channel selection, we used the trained EEG-ARNN to determine which EEG channels are most informative for the MI classification task. We compared Edge Selection (ES), Aggregation Selection (AS), and Gate Selection (GS derived from adaptive gating) on the PhysioNet dataset. For each subject, the network was retrained on the reduced subset while keeping all other parameters constant to ensure fair comparison.

All three strategies preserve accuracy when the channel count is reduced. ES retained an average accuracy between $0.88 \pm 0.03$ (10 channels) and $0.90 \pm 0.03$ (30--40 channels). AS was slightly more stable, reaching $0.89 \pm 0.04$ with 10 channels and plateauing near $0.90 \pm 0.03$ for 20 or more channels. GS offered the best trade-off: with 15--20 channels the model achieved $0.90$--$0.93$ accuracy with deviations below $0.03$, matching or exceeding the full-channel scores.

Across subjects S001--S016, GS showed superior consistency and efficiency. For example, Subject S001 scored $0.8849 \pm 0.0224$ with all channels and $0.8532 \pm 0.0148$ with GS, while S007 scored $0.9365 \pm 0.0245$ full-channel and $0.9325 \pm 0.0245$ with GS. These findings confirm that GS adaptively learns per-subject electrode subsets, often reducing dimensionality by 60--70\% while maintaining or improving accuracy, which is promising for low-channel, real-time BCIs.

\subsection{Comparisons of Classification Performance}

To evaluate EEG-ARNN and the gating-based selection, we compare against EEGNet, FBCNet, a pure CNN, CSP, and mutual-information (MI) selection on PhysioNet data. All models share the same cross-validation folds, measuring accuracy, standard deviation, and retention for Top-10/15/20/25 channels.

Gating-modified EEG-ARNN outperforms these baselines in accuracy and robustness. For Subject S001, EEGNet reached $0.8747 \pm 0.0471$ on 64 channels, whereas GS retained $0.9049 \pm 0.0342$ with 15 channels (>103\% retention). FBCNet hit $0.9179 \pm 0.0386$ full-channel and $0.9006 \pm 0.0385$ at 25 channels, while Pure CNN (\~0.75) and CSP (\~0.64) performed poorly and MI-based selection reached $0.8877 \pm 0.0515$ at 25 channels.

EEG-ARNN yields a 3\%--10\% gain over baselines across subjects S001--S007 for varying channel counts. Keeping 15--20 gate-selected electrodes retains 95\%--105\% of full-channel accuracy with a low std (0.03--0.04), showing consistent generalization; similar trends appear for F1 scores. Tables II and III show EEG-ARNN achieving the highest average accuracy/F1 and lowest variance, with 89\%--92\% at 20--25 channels compared to EEGNet (84\%--87\%), FBCNet (86\%--88\%), and CSP-based models (<80\%). The adaptive gating inside CARM enables subject-specific selection while retaining over 95\% accuracy with roughly one-third of the channels, highlighting its real-time suitability.

The comparison results, summarized in Tables II and III, indicate that EEG-ARNN yields the highest average accuracy and F1-score with the lowest variance. On PhysioNet, EEG-ARNN averaged 89\%--92\% using 20--25 channels, versus EEGNet (84\%--87\%), FBCNet (86\%--88\%), and CSP methods (<80\%). The model is stable across repetitions, with std 0.03--0.04, and GS-driven CARM maintains >95\% accuracy with one-third of the channels, greatly reducing computational costs and enabling real-time decoding. This affirms gating-based EEG-ARNN’s superiority over convolutional and graph baselines for large-scale, cross-subject BCIs.

% tanishq results analysis ends here
...
\begin{figure}[t]
  ...
  \caption{Channel selection frequency across subjects (k=20).}
  \label{fig:channel_freq}
\end{figure}

Analysis of selected channels highlights that sensorimotor electrodes (C3, C4, Cz, CP1, CP2, CPz) remain dominant, while frontal leads (Fz, FC1, FC2) and parietal sites (P3, P4, Pz) appear regularly, reflecting the engagement of motor planning and sensory processing areas. Temporal contacts are chosen less often, indicating their lower relevance for fist MI tasks, and ES/AS exhibit over 80\% overlap, demonstrating agreement on the most informative electrodes.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{results/channel_selection_analysis/reduction_vs_accuracy.png}
  \caption{Accuracy versus channel reduction showing the trade-off curve for ES, AS, and GS strategies.}
  \label{fig:reduction_vs_accuracy}
\end{figure}

\subsection{Subject-Specific Analysis}

Figure \ref{fig:subject_drops} presents accuracy drops for each subject across different k values using adaptive gating with AS.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{results/subject_specific_drops.png}
  \caption{Per-subject accuracy drops for different channel subset sizes (adaptive gating, AS method).}
  \label{fig:subject_drops}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\textwidth]{results/results_summary.png}
  \caption{Summary of baseline versus gated accuracies across subjects (from\_results).}
  \label{fig:results_summary}
\end{figure}

Subject-specific patterns emerge: S007 maintains very high accuracy even with k=10 (drop below 2\%), whereas S002 suffers larger decrements (4-6\%), implying a more distributed signal. Most subjects stabilize for k $\geq$ 15, and a few (S008, S015) even benefit from channel reduction, suggesting the full montage contains noisy entries. These observations reinforce the importance of subject-tailored selection.

\subsection{Comparison of Selection Methods}

Figure \ref{fig:method_comparison} compares the three selection methods (ES, AS, GS) across the baseline and adaptive gating pipelines.
...
Method comparison insights reveal that ES and AS show similar accuracy drops on the baseline architecture, while GS derived from adaptive gating delivers the smallest loss. The gate values serve as a direct ranking of channels, whereas ES/AS continue to be valuable when explicit gate scores are unavailable, offering complementary perspectives derived from the learned adjacency.

\subsection{Computational Efficiency}

Channel reduction brings clear computational benefits: reducing to k=10 channels cuts the input dimension by 84\%, shortens training time by roughly 60-70\%, decreases inference latency by approximately 65-75\%, and proportionally lowers memory and parameter requirements (due to smaller adjacency matrices). These savings are critical for real-time BCI applications and embedded systems.

\section{Discussion}

\subsection{Main Findings}

Our comprehensive evaluation of the baseline and adaptive-gated EEG-ARNN yields several key insights:

\textbf{1. Adaptive gating boosts accuracy.} The adaptive gating module consistently improves or matches the baseline 64-channel performance, reaching a mean accuracy of 84.60\% and capturing per-trial channel saliency.

\textbf{2. Channel selection with minimal accuracy loss.} Gate-based selection derived from adaptive gating keeps 10-20 channels (15-31\% of 64) with only a 0.5--0.7\% drop, while ES/AS on the baseline drops 2.7--4\% for comparable $k$.

\textbf{3. Neurophysiologically valid selection.} Selected channels remain concentrated over sensorimotor and motor planning regions (C3/C4/Cz, CP1/CP2), aligning with known MI activations.

\textbf{4. Method-architecture interactions.} Gate selection (GS) offers the most compact ranking when adaptive gates are available, whereas ES/AS provide useful adjacency-based back-ups when gates are absent.

\textbf{5. Subject-specific variability.} Inter-subject differences in optimal subsets and sensitivity to reduction persist, underscoring the value of subject-wise selection pipelines.
...
The adaptive gating architecture with gate-based selection offers the best accuracy-sparsity trade-off, achieving comparable or better performance than full-channel models using only 15-31\% of channels. These results have important implications for practical BCI systems, potentially enabling more comfortable, portable, and cost-effective brain-computer interfaces for motor rehabilitation and assistive applications.
